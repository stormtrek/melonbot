...
first_hint = threading.Thread()
second_hint = threading.Thread()
current_hint = ''
...
def generate_scramble(channel, irc):
    ...
    while not different:
        while characters:
            scrambled_list.append(characters.pop(random.randrange(len(characters))))
    ...
    if len(word) >= 8:
        ...
        second_hint = threading.Timer(hint_time + 30, show_hint, [word, channel, irc, True])
        second_hint.daemon = True
        second_hint.start()
    else:
        hint_time = 60
        
    first_hint = threading.Timer(hint_time, show_hint, [word, channel, irc])
...
def check_answer(answer, nick, channel, response, irc):
    ...
    if check_for_answer:
        m = re.search('^((melonbot|melontest)(:|,)?)?\s*([^ ]+)$', answer)
        if m:
            answer = m.group(4)
            if answer == word:
                first_hint.cancel()
                if second_hint.isAlive():
                    second_hint.cancel()

                correct_word = word
                
                won = handle_winners(nick)
                if not won:
                    return response
                
                amount_won = len(correct_word) * 10
                money.add(amount_won, nick)

                win_message = '{0}, got it right! The word was \x02{1}\x02. They just won {2} \x0308coins\x03!'.format(nick, correct_word, bold(str(amount_won)))

                if channel not in enabled_channels:
                    send(win_message, irc, enabled_channels[0])
                        
                return win_message
    return response

def handle_winners(nick):
    global word, current_hint, scrambled_word, check_for_answer, second_winner_thread
...
def show_current_scramble():
    global scrambled_word, current_hint
    
    if scrambled_word:
        return 'Unscramble: \x02{0}\x02{1}'.format(scrambled_word, ' | ' + current_hint if current_hint else '')
    else:
        return 'No scrambled word currently in play. Use the .scramble command to generate a new one'

def show_hint(word, channel, irc, second_hint=False):
    global current_hint
    
    if not second_hint:
        hint = 'Hint: {0}'.format(word[0] + (' _' * (len(word) - 1)))
    else:
        hint = 'Hint: {0} {1}'.format(word[0] + (' _' * (len(word) - 2)), word[len(word) - 1])

    current_hint = hint
    send(hint, irc, channel)
